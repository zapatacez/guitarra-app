<script lang="ts">
	...
	let content = $state(song?.content ?? '');
	let selectedKey = $state(song?.key ?? 'C');
	// If the song already has a key saved, treat it as manually set
	let keyManuallySet = $state(!!song?.key);

	$effect(() => {
		if (!keyManuallySet) {
			selectedKey = detectKey(content);
		}
	});
</script>
...
<form method="POST" {action} class="space-y-5">
	<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
		...
		<div>
			<label class="block text-sm text-zinc-400 mb-1" for="key">
				Key
				{#if !keyManuallySet && content}
					<span class="text-zinc-600 ml-1 text-xs">(auto-detected)</span>
				{/if}
			</label>
			<select
				id="key"
