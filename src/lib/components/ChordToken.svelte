<script lang="ts">
	import type { ChordEntry } from '$lib/server/schema';
	import ChordDiagram from './ChordDiagram.svelte';

	let {
		chord,
		chordData
	}: {
		chord: string;
		chordData: ChordEntry | null;
	} = $props();

	let open = $state(false);
	let btnEl: HTMLButtonElement | undefined = $state();

	function toggle() {
		open = !open;
	}

	function onKeyDown(e: KeyboardEvent) {
		if (e.key === 'Escape') open = false;
	}
</script>

<span class="chord-token inline-block relative select-none" onkeydown={onKeyDown}>
	<button
		bind:this={btnEl}
		onclick={toggle}
		class="chord-name font-bold text-amber-400 hover:text-amber-300 active:text-amber-200 cursor-pointer text-sm leading-none px-0.5 rounded transition-colors"
		aria-expanded={open}
		aria-haspopup="dialog"
		type="button"
	>
		{chord}
	</button>

	{#if open && chordData}
		<!-- Backdrop -->
		<div
			role="presentation"
			class="fixed inset-0 z-40"
			onclick={() => (open = false)}
		></div>
		<!-- Popover -->
		<div
			role="dialog"
			aria-label="{chord} chord diagram"
			class="absolute z-50 bottom-full mb-2 left-1/2 -translate-x-1/2 bg-zinc-800 border border-zinc-600 rounded-lg p-3 shadow-xl flex flex-col items-center gap-1"
		>
			<p class="text-white font-bold text-base">{chord}</p>
			<div class="text-zinc-200">
				<ChordDiagram chord={chordData} />
			</div>
			<p class="text-zinc-400 text-xs capitalize">{chordData.difficulty}</p>
		</div>
	{:else if open && !chordData}
		<div
			role="presentation"
			class="fixed inset-0 z-40"
			onclick={() => (open = false)}
		></div>
		<div
			role="dialog"
			class="absolute z-50 bottom-full mb-2 left-1/2 -translate-x-1/2 bg-zinc-800 border border-zinc-600 rounded-lg px-3 py-2 shadow-xl"
		>
			<p class="text-zinc-400 text-xs">{chord}</p>
			<p class="text-zinc-500 text-xs">No diagram</p>
		</div>
	{/if}
</span>
